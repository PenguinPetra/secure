...
export default async function DashboardPage() {
  ...
  const currentSession = await prisma.session.findUnique({ where: { id: sessionId }, include: { user: true } });
          ...
          {activeSessions.map((session) => (
            <div key={session.id} className="flex items-center justify-between border-b pb-2">
              <div>
                ...
                <p className="text-xs text-gray-500">IP: {session.ipAddress} | 開始: {session.createdAt.toLocaleString()}</p>
                {session.id === sessionId && <span className="text-xs bg-blue-100 text-blue-600 px-2 py-0.5 rounded">このデバイス</span>}
              </div>
          ...
          <tbody>
            {histories.map((log) => (
              <tr key={log.id} className="border-b">
                ...
                <td><span className={log.status === "SUCCESS" ? "text-green-600" : "text-red-600"}>{log.status}</span></td>
                <td>{log.ipAddress}</td>
              </tr>
            ))}
          </tbody>
